...
A confusion matrix is a table used to evaluate the performance of a classification model by comparing actual target values with the model’s predictions. It summarises outcomes as true positives (TP), where ICH is correctly predicted; false positives (FP), where ICH is predicted but not present; true negatives (TN), where ICH is correctly predicted as absent; and false negatives (FN), where ICH is present but not predicted.
